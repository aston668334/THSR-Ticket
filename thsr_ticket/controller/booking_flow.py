...
from thsr_ticket.model.web.booking_form.ticket_num import AdultTicket, CollegeTicket
...
from thsr_ticket.Resnet.captcha_code import Resnet, get_captcha_code
...
class BookingFlow:
    ...
    def run(self) -> Response:
        ...
        ticket_type = input('請問要訂 1:成人、2:大學生優惠票:')
        if ticket_type == '2':
            self.set_college_ticket_num()
        else:
    ...
    def set_college_ticket_num(self) -> None:
        if self.record.college_num is not None:
            self.book_form.college_ticket_num = self.record.college_num
        else:
            sel = self.book_info.ticket_num_info("學生優惠票", default_value=1)
            self.book_form.college_ticket_num = CollegeTicket().get_code(sel)
            #self.book_form.adult_ticket_num = AdultTicket().get_code(0)
    ...
    def set_adult_ticket_num(self) -> None:
        if self.record.adult_num is not None:
            ...
            #self.book_form.college_ticket_num =  CollegeTicket().get_code(0)
...
#         plt.show()
#         return input()
        out = get_captcha_code(image,self.model)
        print(out)
        return out
